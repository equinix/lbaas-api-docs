...
```bash
curl -X POST -H 'Content-Type: application/json' -H "Authorization: bearer < access_token >" https://lb.metalctrl.io/v1/projects/< project id>/loadbalancers -d '{ "name": "test-loadbalancer", "location_id": "<location id >"}'
```
#### Fields
...
| provider_id     | ID     |    -     | ID of the provider to deploy the loadbalancer. Currently only `loadpvd-gOB_-byp5ebFo7A3LHv2B` is supported                            |
